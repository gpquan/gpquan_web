  ...
  <div class="container">
    ...
    <div class="head_card">
      <div>
        ...
        <nut-button type="light" shape="circle" small id="btn">删除项目</nut-button>
      </div>
      <div class="head_crad_text">
        <div
          class="head_card_icon"
          :style="'background-image:url('+progressObj[imgArr[this.$route.query.type]]+')'"
        ></div>
        <div class="head_card_left">
          <p
            v-text="progressObj.name"
            style="font-size: 16px;color: #fff;"
            @click="Jump_organ(progressObj.organ_id,progressObj.project_id)"
          ></p>
          ...
          <nut-button
            ...
            shape="circle"
            small
            v-for="(item,ind) in  (typeof(progressObj.lingyu_name) == 'string' ? [{name:progressObj.lingyu_name}] : progressObj.lingyu_name)  "
            :key="ind"
            v-if="ind < 3"
            :color="colorList[ind]"
          ...
          <p
            v-if="pageType == 0"
            style="margin-top: 1vmin;color: #fff;"
          >佣金比例：{{progressObj.yongjin}}%</p>
        </div>
      </div>
    </div>
        ...
        <div class="step-item-box" v-for="(item,ind) in progressObj.progress" :key="ind">
          ...
          >
            <div class="step-circle">
              <span class="step-circle-inner"></span>
            </div>
          </div>
          <div class="step-item-right" @click="showPop(1,item.id)">
            ...
            <div class="step-right-cont">
              <p v-if="sta">
                <span class="cont-circle"></span>我是备注备注
              </p>
            </div>
          </div>
        </div>
    ...
    <div class="btn" @click="back">
      <nut-button block shape="circle">返回</nut-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDetails: function() {
      ...
      this.$post(
        "/api/" + ["getProjectInvestProgress", "getOrganInvestProgress"][type],
        obj
      ).then(res => {
