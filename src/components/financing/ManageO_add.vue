  ...
  <div v-if="pageStep == 1">
    ...
    <div class="list-body">
      ...
      <div @click="gopersonage()">
        <nut-cell
          title="项目简介"
          :desc="description ? description : '请输入项目简介'"
          :showIcon="true"
          class="projectJS"
        ></nut-cell>
      </div>
...
export default {
  ...
  beforeMount() {
      ...
      this.ProID=this.$route.query.typeID
   }
      ...
      this.$post("/api/addOrgan", {
		...
		if(this.ProID){
     		this.$post('/api/addUserOrganProject',{
				 ...
				 console.log(res)
				 this.$router.push('/accelerate/Manage/p')
			 })
		}else{
			this.$post("/api/addUserOrgan", {
				 organId:OrganId,
				 userId:this.userId,
			 }).then((res)=>{
				 console.log(res)
